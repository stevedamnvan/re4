...
# The first compact contract is deliberately r100 only. These are source SMD
# scenery/model palettes and the room effect texture table, not core noise,
# card images, font data, palettes, or mip chains. Keep the other resource
# families resident until their CPU consumers and native formats are qualified.
...
def compact_room(source_file, textures, destination):
    ...
    for tpl_off,raw,ctx in palettes:
        allowed=ctx in (arc+'#5/TPL0',arc+'#10/item') or any(ctx==arc+'#'+str(i) for i in model_slots)
        if ctx.startswith(arc+'#8/tpl'):
            ident=int(ctx.rsplit('tpl',1)[1])
            # Espgen42/45 consume 0xFE as CPU noise. Never externalize it.
            allowed=ident<len(ids) and ids[ident]!=0xfe
        if not allowed:
            continue
    ...
    report={'contract':'r100-upload-only-v1','source_file':str(source_file),
            ...
            'original_archive_bytes':len(decoded),'resident_archive_bytes':len(out),
...
if __name__=='__main__':
    if '--compact-room' in sys.argv:
        import argparse
        parser=argparse.ArgumentParser(description='Compact qualified r100 upload-only textures using existing native packages')
        parser.add_argument('--compact-room',type=Path,required=True)
        parser.add_argument('--textures',type=Path,required=True)
        parser.add_argument('--output',type=Path,required=True)
        args=parser.parse_args()
        report=compact_room(args.compact_room,args.textures,args.output)
        print('compact r100:',report['original_archive_bytes'],'->',report['resident_archive_bytes'],
