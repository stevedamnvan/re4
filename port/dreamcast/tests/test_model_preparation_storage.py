...
#include <cstring>
#include <cstdint>
struct MemHeap {int handle;}; MemHeap Heap[13];
int current=4,active=1,free_bytes=213824,allocations=0,frees=0,detaches=0;
bool fail=false,live=false; alignas(32) unsigned char storage[131072];
...
void Mem_free_h(void* p,int heap){assert(p==storage && heap==4 && live && detaches>frees);++frees;live=false;free_bytes+=sizeof(storage)+64;}
...
int main(){unsigned bytes=1;
 ...
 for(unsigned cycle=0;cycle<3;++cycle){
  ...
  assert(re4dc_model_retained_storage(&bytes)==storage && bytes==131072 && free_bytes==82688);
  const int count=allocations;storage[0]=7;
  assert(re4dc_model_retained_storage(&bytes)==storage && storage[0]==7 && allocations==count);
 ...
 assert(re4dc_model_retained_storage(&bytes)==storage && allocations==5);
 re4dc_model_preparation_owner((void*)7);assert(!live && frees==4);
 assert(re4dc_model_retained_storage(&bytes)==storage && allocations==6);
 re4dc_model_preparation_owner(nullptr);assert(!live && free_bytes==213824);
}
"""
