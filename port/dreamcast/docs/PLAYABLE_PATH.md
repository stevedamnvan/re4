...
- The four-sample Leon fixture proves skeleton/motion evaluation. Visible
  skinned Leon, continuous locomotion, collision, and combat remain unbuilt.
...
|---|---|---|
| P0 - walkable room | Integrate Maple movement, a shoulder follow camera, SAT floor/wall collision, spawn, and an exit/reset trigger in one native executable. A capsule or debug skeleton is sufficient during integration. Use current scenery, with a proxy display only if necessary to keep this step runnable. | A recorded controller route moves, turns, stops at walls, follows the intended floor, reaches the exit, and resets three times without drift or growing allocations. This is the walkable checkpoint, not accepted RE4 gameplay. |
...
Start P0: decode the existing private `r10d_001.SAT`, verify its coordinate
mapping against the exported room, and add floor/wall movement plus a follow
camera to the native executable. Use the preserved-group package for the
bounded comparison. Keep the old viewer and evidence reproducible.

Before preparing the P2 Ganado, inspect existing model variants and source
selection logic for a compatible cheaper mesh. The developer postmortem points
to existing detail tiers; their availability in this debug disc is still to
be verified. Preserve necessary material/vertex color data during conversion.
