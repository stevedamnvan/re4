...
|---|---|---|
| Room and material selection | Original data, converted offline | Main/shared SMD plus BLK-resident FILE_00/01/02; retain intact selected meshes and repair material IDs from ModelPart headers. Opaque source textures use RGB565. Textures carrying source alpha use ARGB4444. The twelve gradient-alpha materials retain source-alpha/inverse-source-alpha blending. `ROOM_MATERIAL_029` alone combines binary alpha with the source SMX bit-3 `alpha_omit = 0x80` override, so it uses the Dreamcast punch-through list at the same reference; binary material 008 lacks that override and remains blended until its per-part source reference is recovered. |
