...
| Player and Ganado state machines | Mixed adaptation and temporary approximation | Both actors resolve against the shared source SAT walls, and shot segments are rejected by intervening wall triangles. The starting handgun refills at source reload frame 44 and stays locked through the source frame-55 pin event. Direct pursuit, enemy attack timing, and the reduced input/state layer remain explicit port debt. |
| HUD and sound | Mixed source conversion and temporary presentation layer | HUD is native and readable. The GameCube weapon archive's original DSP-ADPCM cues are converted offline: `cObjMauser::moveFire` cues 0 and 2 play together, and the starting reload uses cue `0x16`. Enemy, impact, and room sound remain open. |
