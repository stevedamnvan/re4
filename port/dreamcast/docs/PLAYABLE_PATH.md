...
| Camera and fog | Original data with Dreamcast projection adaptation | Use r100 CAM area 2 cut 2 offsets/FOV, the global handgun-ready offsets/FOV, and r100 LIT fog/background values. While aiming, `CameraQuasiFPS::calcOffset` is preserved as a linear blend across the exact three `g_readyOfs[0][0]` camera/target entries, driven by the same pitch value as Leon's weapon motion. |
