...
4. Run the R4 asset residency and streaming workstream alongside the remaining frame work, in the order of `port/dreamcast/docs/R4_ASSET_RESIDENCY_PLAN.md`: asset inventory tool (GameCube vs PS2 vs Dreamcast candidates per asset), offline native texture layout and deduped handles, package-resident batch-local tables, residency model from the authored block sets, asynchronous reads into a staging arena, one room transition with eviction. GameCube is the authority, PS2 is an oracle for Capcom's reductions only, DCA3 is the Dreamcast streaming prior art (mechanisms, not code or data).
